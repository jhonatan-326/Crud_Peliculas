...
// ================== Carrusel y Funcionalidad de Flechas ================== //
...
function renderizarSlider() {
    ...
    const recientes = peliculasGlobales.slice(0, 5);
...
function renderizarSlider() {
    ...
    // 1. Triplicamos las películas para crear el efecto infinito (Bloque 1, Bloque 2 y Bloque 3)
    ...
    // Calculamos el ancho de un set completo de películas
    const anchoUnSet = peliculas.length * 260; // 240px tarjeta + 20px gap

    // Posicionamos el scroll exactamente en el inicio del SEGUNDO bloque al cargar
    carrusel.scrollLeft = anchoUnSet;

    // Escucha el movimiento para hacer el salto invisible (infinito)
    carrusel.onscroll = () => {
        // Si llega muy a la derecha (entra al tercer bloque), salta al inicio del segundo bloque
        if (carrusel.scrollLeft >= anchoUnSet * 2) {
            carrusel.style.scrollBehavior = 'auto'; // Quita la animación momentáneamente
            carrusel.scrollLeft -= anchoUnSet;
            carrusel.style.scrollBehavior = 'smooth'; // Restaura la animación suave
        }
